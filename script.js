...
const firebaseConfig = {
    ...
    measurementId: "G-DQCJLHT8LY"
        ...
        // Separate Google Login elements
        ...
        const googleUserBox = document.getElementById('googleUserBox');
        ...
        // Separate Google Login: does NOT change the existing Admin Login
        googleLoginBtn.addEventListener('click', () => {
            googleLoginModal.classList.add('active');
            googleLoginMessage.textContent = '';
        });

        closeGoogleModal.addEventListener('click', () => googleLoginModal.classList.remove('active'));
        window.addEventListener('click', (e) => {
            if (e.target === googleLoginModal) googleLoginModal.classList.remove('active');
        });

        googleSignInBtn.addEventListener('click', async () => {
            googleLoginMessage.style.color = '';
            googleLoginMessage.textContent = 'Opening Google login...';
            try {
                ...
                googleLoginMessage.style.color = '#18a558';
                googleLoginMessage.textContent = 'Google login successful.';
            } catch (error) {
                console.error('Google login error:', error);
                googleLoginMessage.style.color = '#ff4444';
                googleLoginMessage.textContent = error.message || 'Google login failed.';
            }
        });
        ...
        googleLogoutBtn.addEventListener('click', async () => {
            try {
                ...
                googleLoginMessage.style.color = '#ff4444';
                googleLoginMessage.textContent = error.message || 'Logout failed.';
            }
        });
        ...
        auth.onAuthStateChanged((user) => {
            if (user) {
                ...
                googleUserBox.style.display = 'block';
                googleUserName.textContent = user.displayName || 'Google User';
                googleUserEmail.textContent = user.email || '';
                googleUserPhoto.src = user.photoURL || '';
                googleLoginBtn.innerHTML = '<i class="fab fa-google"></i> ' + (user.displayName ? user.displayName.split(' ')[0] : 'Google Account');
            } else {
                googleSignInBtn.style.display = 'block';
                googleUserBox.style.display = 'none';
                googleUserName.textContent = '';
                googleUserEmail.textContent = '';
                googleUserPhoto.src = '';
                googleLoginBtn.innerHTML = '<i class="fab fa-google"></i> Google Login';
            }
        });
